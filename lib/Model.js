...
class Model {
   ...
   */
  static create (name, url, fields, options) {
    options = options || {}
    ...
    class GoCardlessModel {
      ...
      static list (parameters, callback) {
        GoCardless.client().get({
          ...
          callback(null, {
            ...
            before: body.meta.cursors.before,
            after: body.meta.cursors.after,
            results: _.map(body[url], (m) => new GoCardlessModel(m))
          })
        })
      }
      ...
      save (callback) {
        this.validate((err) => {
          ...
          if (this._data.id) {
            if (!options.update) {
              json[url] = _.omit(json[url], ['id', 'created_at'])
            } else {
              json[url] = _.pick(json[url], options.update)
            }
            ...
            if (!options.create) {
              ...
              json[url] = _.pick(json[url], options.create)
            }
    ...
    _.each(options.methods || {}, (method, name) => {
      GoCardlessModel.prototype[name] = method;
    })

    _.each(options.statics || {}, (method, name) => {
      GoCardlessModel[name] = method;
    })
